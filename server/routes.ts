...
const getApiUrl = (): string => {
  return (
    process.env.API_URL || "http://localhost:8082/fitly"
  );
};
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // === Payment API Routes ===
  app.post("/api/payment/createcheckoutsession", async (req: Request, res: Response) => {
    try {
      const apiUrl = getApiUrl();
      const response = await axios.post(
  ...
  app.post("/api/payment/customerPortal", async (req: Request, res: Response) => {
    try {
      const apiUrl = getApiUrl();
      const response = await axios.post(

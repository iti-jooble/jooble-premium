import { CV, PersonalInfo, WorkExperience, Education, Skill } from "@shared/schema";
 ...
 */
export function calculateCvScore(cv: CV): number {
  if (!cv) return 0;
  
  const sectionWeights: SectionScores = {
    personalInfo: 30, // Most important section
    workExperience: 30, // Equally important
    education: 15,
    skills: 15,
    summary: 10
  };
  ...
  const personalInfoScore = calculatePersonalInfoScore(cv.personalInfo) * sectionWeights.personalInfo;
  
  // Calculate work experience score (30%)
  const workExperienceScore = calculateWorkExperienceScore(cv.workExperience) * sectionWeights.workExperience;
  
  // Calculate education score (15%)
  const educationScore = calculateEducationScore(cv.education) * sectionWeights.education;
  ...
  const summaryScore = calculateSummaryScore(cv.summary) * sectionWeights.summary;
  
  // Calculate total score (0-100)
  const totalScore = Math.round(
    personalInfoScore + workExperienceScore + educationScore + skillsScore + summaryScore
  );
 ...
 */
function calculatePersonalInfoScore(personalInfo?: PersonalInfo): number {
 ...
 */
function calculateWorkExperienceScore(experiences?: WorkExperience[]): number {
  ...
  const avgCompleteness = experiences.length > 0 ? completenessSum / Math.min(experiences.length, 3) : 0;
...
function calculateEducationScore(educations?: Education[]): number {
  ...
  const avgCompleteness = educations.length > 0 ? completenessSum / Math.min(educations.length, 2) : 0;
